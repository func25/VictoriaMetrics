...
func GetMetricNamesUsageStats(qt *querytracer.Tracer, limit, le int, matchPattern string) (storage.MetricNamesUsageStatsResponse, error) {
	qt = qt.NewChild("get metric names usage stats with limit: %d, less equal to: %d, match pattern=%q", limit, le, matchPattern)
	defer qt.Done()
	return vmstorage.GetMetricNamesUsageStats(qt, limit, le, matchPattern)
}
