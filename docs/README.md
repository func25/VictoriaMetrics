...
```
 ...
 `VictoriaMetrics` stores tracked metric names in-memory and saves state `on-disk` at `data/cache` folder. State could be reset via API call `/api/v1/admin/tsdb/reset_metric_names_usage` or per storage via [cache removal](#cacheremoval).
